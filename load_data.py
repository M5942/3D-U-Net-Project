...
# TODO: Separate test and train data into separate folders
def get_data(data_dir: str, patch_size: tuple = (64, 64, 64), test_size: float = 0.3):
    ...
    X_train = np.array([load_nifti(image) for image in image_paths_train], dtype=object)
    y_train = np.array([load_nifti(label) for label in label_paths_train], dtype=object)
    X_test = np.array([load_nifti(image) for image in image_paths_test], dtype=object)
    y_test = np.array([load_nifti(label) for label in label_paths_test], dtype=object)
...
if __name__ == '__main__':
    ...
    X_train, X_test, y_train, y_test = get_data(root_dir, patch_size=(64, 64, 64), test_size=0.3)
