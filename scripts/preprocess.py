...
def get_patches(image: np.ndarray, patch_size: tuple[int, int, int] = (64, 64, 64), stride: int = 64) -> np.ndarray:
    ...
    # Get the patches. A stride of 64 means there will be no overlap between patches
...
def preprocess_mask(mask: np.ndarray, new_dims: np.ndarray) -> np.ndarray:
    ...
    # Resizing the mask changes the values of the pixels. We need to change them back to 0 and 1
    # Set all the values greater than 0 to 1
    mask[mask != 0] = 1
